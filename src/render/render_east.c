...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/16 12:53:19 by lpaysant          #+#    #+#             */
/*   Updated: 2025/10/16 15:42:39 by lpaysant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../header/cub3D.h"

t_argb	change_pxl_east(t_all *all, t_render *render, float impact, int z)
{
	t_argb	color;
...
void	init_render_east_values(t_all *all, t_render *render,
		int *z, t_argb *color)
{
	render->lost_pix = 0;
	render->to_draw = render->draw_end - render->draw_start;
	render->text_to_put = (render->text_perc / 100) * all->texture->height_n;
	render->coef_pix = render->to_draw / render->text_to_put;
	render->start_on_texture = (all->texture->height_n
...
void	render_east(t_all *all, t_render *render, int x, int z)
{
	...
	impact = 1;
	z = 0;
	init_render_east_values(all, render, &z, &color);
	y = render->draw_start;
	while (y < render->draw_end)
	{
		if (z % (int)render->coef_pix == 0)
			color = change_pxl_east(all, render, impact, z);
