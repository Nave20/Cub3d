...
/*   Updated: 2025/11/11 15:09:26 by lpaysant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_render_n(t_all *all, t_render *render)
{
	...
	render->coef_pix = (float)all->data->texture->height_n
	...
	render->impact = (float)all->data->texture->width_n
		- select_impact(all->data)
		* (float) all->data->texture->width_n;
}

static void	pixel_loop_n(t_all *all, t_render *render, float texpos, int x)
{
	t_argb	color;
	int		i;
