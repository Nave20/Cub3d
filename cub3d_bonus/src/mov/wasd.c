...
void	s_key(t_data *data, float new_x, float new_y)
{
	...
	if (data->map[(int)new_y][(int)player->pos_x] != '1' && data->map[(int)new_y][(int)player->pos_x] != 'C')
		data->player->pos_y = new_y;
	new_x -= STEP * ft_trig(data, player->radian, SIN);
	if (data->map[(int)player->pos_y][(int)new_x] != '1' && data->map[(int)player->pos_y][(int)new_x] != 'C')
